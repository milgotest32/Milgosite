...
import KonumGostergesi from '../ui/KonumGostergesi'
...
export default function Navbar() {
  ...
  const S: Record<string, React.CSSProperties> = {
    drop: { position: 'absolute', top: 'calc(100% + 8px)', background: '#fff', borderRadius: '20px', boxShadow: '0 12px 40px rgba(26,10,18,0.15)', border: '1px solid rgba(232,86,122,0.08)', padding: '8px', minWidth: '190px', zIndex: 200 },
    di: { display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 12px', borderRadius: '10px', fontSize: '13px', fontWeight: 500, color: '#1A0A12', textDecoration: 'none', background: 'none', border: 'none', width: '100%', cursor: 'none', fontFamily: 'Syne, sans-serif' } as React.CSSProperties,
    icon: { width: '38px', height: '38px', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '10px', color: '#7A6070', background: 'transparent', border: 'none', cursor: 'none', flexShrink: 0 } as React.CSSProperties,
  }
  ...
  return (
    <>
      ...
      <nav style={{ position: 'sticky', top: 0, zIndex: 100, backdropFilter: 'blur(20px)', background: 'rgba(253,251,249,0.95)', borderBottom: '1px solid rgba(232,86,122,0.08)', boxShadow: '0 1px 16px rgba(26,10,18,0.04)' }}>
        <div style={{ maxWidth: '1280px', margin: '0 auto', padding: '0 clamp(16px,4vw,48px)', height: '64px', display: 'flex', alignItems: 'center', gap: '8px' }}>

          {/* Logo */}
          <Link href="/" style={{ fontFamily: '"Instrument Serif", serif', fontSize: '26px', color: '#1A0A12', textDecoration: 'none', flexShrink: 0, marginRight: '4px' }}>
                ...
                {urunDrop && (
                  <div style={S.drop}>
                    {[{ e: '🥛', a: 'Çiğ Süt', h: '/kategoriler/cig-sut' }, { e: '🧀', a: 'Peynir', h: '/kategoriler/peynir' }, { e: '🧈', a: 'Tereyağı', h: '/kategoriler/tereyagi' }].map(k => (
                      <Link key={k.h} href={k.h} style={S.di as any}>{k.e} {k.a}</Link>
                    ))}
                    <div style={{ borderTop: '1px solid rgba(26,10,18,.06)', margin: '6px 0 0', paddingTop: '6px' }}>
                      <Link href="/kampanyalar" style={{ ...S.di, color: '#E8567A', fontWeight: 700 } as any}>🔥 Kampanyalar</Link>
                    </div>
                  </div>
                )}
              </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '4px', marginLeft: 'auto' }}>
            <button onClick={() => setAramaAcik(!aramaAcik)} style={S.icon}><Search size={17} strokeWidth={1.75} /></button>
            <Link href="/hesabim/favoriler" style={{ ...S.icon, textDecoration: 'none' } as any}><Heart size={17} strokeWidth={1.75} /></Link>

            {/* User dropdown */}
            <div style={{ position: 'relative' }}>
              <button onClick={() => setUserDrop(!userDrop)} style={S.icon}><User size={17} strokeWidth={1.75} /></button>
              {userDrop && (
                <div style={{ ...S.drop, right: 0, left: 'auto' }}>
                  {user ? (
                    <>
                      ...
                      {[{ i: <Package size={14} />, a: 'Siparişlerim', h: '/hesabim/siparisler' }, { i: <Heart size={14} />, a: 'Favorilerim', h: '/hesabim/favoriler' }, { i: <MapPin size={14} />, a: 'Adreslerim', h: '/hesabim/adresler' }, { i: <Settings size={14} />, a: 'Hesabım', h: '/hesabim' }].map(item => (
                        <Link key={item.h} href={item.h} onClick={() => setUserDrop(false)} style={{ ...S.di, color: '#7A6070' } as any}><span>{item.i}</span>{item.a}</Link>
                      ))}
                      <div style={{ borderTop: '1px solid rgba(26,10,18,.06)', marginTop: '6px', paddingTop: '6px' }}>
                        <Link href="/admin" onClick={() => setUserDrop(false)} style={{ ...S.di, color: '#7A6070' } as any}>⚙️ Admin Panel</Link>
                        <button onClick={cikis} style={{ ...S.di, color: '#E8567A' } as any}><LogOut size={14} />Çıkış Yap</button>
                      </div>
                    </>
                  ) : (
                    <>
                      <Link href="/giris" onClick={() => setUserDrop(false)} style={{ ...S.di, color: '#E8567A', fontWeight: 700 } as any}>Giriş Yap</Link>
                      <Link href="/kayit" onClick={() => setUserDrop(false)} style={S.di as any}>Üye Ol</Link>
                    </>
                  )}
                </div>
              )}
            ...
            {/* Sepet */}
            <button onClick={() => setSepetAcik(true)} style={{ display: 'flex', alignItems: 'center', gap: '7px', background: '#FEE8EF', padding: '8px 14px', borderRadius: '12px', border: 'none', cursor: 'none', flexShrink: 0 }}>
            ...
            {/* Hamburger */}
            {isMobile && (
              <button onClick={() => setMenuAcik(!menuAcik)} style={S.icon}>
        ...
        {/* Arama */}
        {aramaAcik && (
          <div style={{ borderTop: '1px solid rgba(232,86,122,.08)', padding: '10px clamp(16px,4vw,48px)', background: 'rgba(253,251,249,0.98)' }}>
      ...
      {/* Mobil menü */}
      {menuAcik && isMobile && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 150, background: '#FDFBF9', overflowY: 'auto', padding: '80px 24px 40px', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ fontFamily: '"Instrument Serif", serif', fontSize: '24px', color: '#1A0A12', marginBottom: '28px' }}>
            milgo<span style={{ color: '#E8567A' }}>.</span>
          </div>
          <div style={{ flex: 1 }}>
            {[['🥛 Çiğ Süt', '/kategoriler/cig-sut'], ['🧀 Peynir', '/kategoriler/peynir'], ['🧈 Tereyağı', '/kategoriler/tereyagi'], ['🛍 Tüm Ürünler', '/urunler'], ['🔄 Abonelik', '/abonelik'], ['🌿 Çiftliğimiz', '/ciftligimiz'], ['📖 Blog', '/blog'], ['🔥 Kampanyalar', '/kampanyalar']].map(([a, h]) => (
              <Link key={h} href={h} onClick={() => setMenuAcik(false)} style={{ display: 'block', padding: '15px 0', fontSize: '18px', fontWeight: 500, color: '#1A0A12', textDecoration: 'none', borderBottom: '1px solid rgba(26,10,18,.06)' }}>{a}</Link>
            ))}
          </div>
