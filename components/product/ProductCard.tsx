...
export default function ProductCard({ urun, sezonDisi }: { urun: Urun; sezonDisi?: boolean }) {
  ...
  const favoriToggle = (e: React.MouseEvent) => {
    e.preventDefault()
    toggle(urun.id)
    toast(favori ? 'Favorilerden çıkarıldı' : 'Favorilere eklendi!', { icon: favori ? '🤍' : '❤️' })
  }
    ...
    <Link href={`/urun/${urun.slug}`} className="card-2026" style={{ display: 'block', textDecoration: 'none', borderRadius: '32px', overflow: 'hidden' }}>
      ...
      <div style={{ padding: '16px 18px 18px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <button onClick={sepeteEkle} disabled={urun.stok_takip && urun.stok <= 0} className="prod-add" style={{ background: '#1A0A12' }}>+</button>
        </div>
      </div>
    </Link>
  )
