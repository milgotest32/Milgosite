...
export default function WhatsAppButon() {
  ...
  useEffect(() => {
    supabase.from('site_ayarlar').select('anahtar,deger').eq('grup', 'whatsapp')
      .then(({ data }) => {
        const a: Record<string, string> = {}
        data?.forEach((r: any) => { a[r.anahtar] = r.deger || '' })
        setNumara(a.numara || '')
        setYazi(a.yazi || 'Sipariş İçin Whatsapp')
        setMesaj(a.mesaj || '')
      })
  }, [])
  ...
  const url = `https://wa.me/${numara}${mesaj ? `?text=${encodeURIComponent(mesaj)}` : ''}`
  ...
  return (
    <>
      ...
      <a href={url} target="_blank" rel="noreferrer" className="wa-btn">
        ...
        <span className="wa-label">{yazi}</span>
      </a>
    </>
  )
}
