...
export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
  ...
  const { data } = await db.from('site_blog_yazilar').select('baslik,ozet,seo_title,seo_description,gorsel_url').eq('slug', params.slug).single()
...
export default async function BlogDetayPage({ params }: { params: { slug: string } }) {
  const db = createServerClient()
  const { data: yazi } = await db.from('site_blog_yazilar').select('*,site_blog_kategoriler(name),site_users(ad,soyad)').eq('slug', params.slug).single()
      ...
      <div style={{maxWidth:'760px',margin:'0 auto'}}>
        ...
        <div style={{background:'#fff',borderRadius:'24px',overflow:'hidden',border:'1px solid #F0ECF5'}}>
          ...
          <div style={{padding:'36px'}}>
            {yazi.site_blog_kategoriler && <span style={{fontSize:'10px',fontWeight:700,background:'#FEF0F4',color:'#E07090',padding:'4px 12px',borderRadius:'50px'}}>{yazi.site_blog_kategoriler.name}</span>}
            ...
            <div style={{display:'flex',alignItems:'center',gap:'16px',fontSize:'12px',color:'#9CA3AF',marginBottom:'24px',paddingBottom:'24px',borderBottom:'1px solid #F0ECF5'}}>
              {yazi.site_users && <span>{yazi.site_users.ad} {yazi.site_users.soyad}</span>}
              <span>{new Date(yazi.created_at).toLocaleDateString('tr-TR',{day:'numeric',month:'long',year:'numeric'})}</span>
              <span style={{display:'flex',alignItems:'center',gap:'4px'}}><Clock size={12}/>{yazi.okuma_suresi} dk okuma</span>
            </div>
