...
export default function AdminUrunlerPage() {
  ...
  const DURUM_RENK: Record<string,{bg:string,tx:string}> = {
    active:{bg:'#F0FDF4',tx:'#22C55E'}, draft:{bg:'#F8F7FC',tx:'#9CA3AF'}, deleted:{bg:'#FEF2F2',tx:'#EF4444'}
  }
                ...
                <tr key={u.id} style={{borderBottom:'1px solid #F0ECF5',background:i%2===0?'#fff':'#FAFAF9'}}>
                  ...
                  <td style={{padding:'12px 16px'}}>
                    <div style={{display:'flex',gap:'6px'}}>
                      ...
                      <button onClick={()=>sil(u.id,u.name)} style={{width:'30px',height:'30px',borderRadius:'8px',background:'#FEF2F2',display:'flex',alignItems:'center',justifyContent:'center',color:'#EF4444',border:'none',cursor:'none'}}><Trash2 size={13}/></button>
                    </div>
                  </td>
                </tr>
              )
