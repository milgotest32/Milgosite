...
export default function KategorilerPage() {
  ...
  const sil = async(id:string,name:string)=>{ if(!confirm(`"${name}" silinsin mi?`))return; await supabase.from('site_kategoriler').delete().eq('id',id); toast.success('Silindi'); yukle() }
  ...
  return (
    <div>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'24px'}}>
        ...
        <button onClick={()=>setForm({...form,goster:true})} style={{display:'flex',alignItems:'center',gap:'8px',background:'linear-gradient(135deg,#E07090,#3B9FCC)',color:'#fff',padding:'10px 20px',borderRadius:'50px',border:'none',fontSize:'13px',fontWeight:700,cursor:'none',fontFamily:'inherit'}}><Plus size={15}/>Ekle</button>
      </div>
      {form.goster&&(<div style={{background:'#fff',borderRadius:'16px',border:'2px solid #F4A7B9',padding:'24px',marginBottom:'16px'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'12px',marginBottom:'12px'}}>
          {[['Ad *','name'],['Slug','slug'],['Açıklama','aciklama'],['Sıra','sira']].map(([l,k])=>(
            ...
            <input value={(form as any)[k]} onChange={e=>setForm({...form,[k]:e.target.value})} style={{width:'100%',background:'#F8F7FC',border:'1px solid #F0ECF5',borderRadius:'10px',padding:'10px 14px',fontSize:'13px',outline:'none',fontFamily:'inherit'}}/></div>
          ))}
        </div>
            ...
            :kategoriler.map((k,i)=>(
              <tr key={k.id} style={{borderBottom:'1px solid #F0ECF5',background:i%2===0?'#fff':'#FAFAF9'}}>
                ...
                <td style={{padding:'12px 16px'}}>
                  <button onClick={()=>toggleAktif(k.id,!k.aktif)} style={{fontSize:'11px',fontWeight:700,padding:'3px 10px',borderRadius:'50px',border:'none',cursor:'none',background:k.aktif?'#F0FDF4':'#FEF2F2',color:k.aktif?'#22C55E':'#EF4444'}}>{k.aktif?'Aktif':'Pasif'}</button>
                </td>
                <td style={{padding:'12px 16px'}}>
                  <button onClick={()=>sil(k.id,k.name)} style={{width:'30px',height:'30px',background:'#FEF2F2',border:'none',borderRadius:'8px',color:'#EF4444',cursor:'none',display:'flex',alignItems:'center',justifyContent:'center'}}><Trash2 size={13}/></button>
                </td>
              </tr>
            ))}
          </tbody>
