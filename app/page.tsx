...
export default function AnaSayfa() {
  ...
  const C: React.CSSProperties = { width: '100%', height: '100%', objectFit: 'contain', padding: '32px', animation: 'float 6s ease-in-out infinite', mixBlendMode: 'multiply' }
  ...
  return (
    <div style={{ background: '#FDFBF9', overflowX: 'hidden' }}>
      ...
      <section style={{ position: 'relative', overflow: 'hidden' }}>
        ...
        <div className="hero-grid" style={{ minHeight: '88vh', position: 'relative', zIndex: 2 }}>
          {/* Sol */}
          <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: 'clamp(40px,6vw,80px)' }}>
            ...
            <h1 style={{ fontFamily: '"Instrument Serif", serif', fontSize: 'clamp(48px, 7vw, 88px)', lineHeight: .95, color: '#1A0A12', marginBottom: '20px', letterSpacing: '-.02em' }}>
              Mutlu&shy;luğun<br />
          ...
          <div className="hero-right-desktop" style={{ position: 'relative', overflow: 'hidden' }}>
            <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '60px' }}>
              <div style={{ width: '100%', height: '100%', borderRadius: '0 0 0 80px', overflow: 'hidden', background: 'linear-gradient(135deg,#FEE8EF,#EBF5FC)' }}>
                <img src="https://market.milgo.com.tr/cdn/shop/files/Milgo_UrunGorselleri_CigSut_1260x1600px_1.jpg" alt="Milgo" style={C} />
              </div>
            </div>
            {/* Float kartlar */}
          ...
          {/* Mobil görsel */}
          <div className="hero-right-mobile" style={{ background: 'linear-gradient(135deg,#FEE8EF,#EBF5FC)', alignItems: 'center', justifyContent: 'center', padding: '28px 40px', minHeight: '260px' }}>
      ...
      {/* KATEGORİLER */}
      <section style={{ padding: 'clamp(40px,6vw,80px) clamp(16px,5vw,80px) clamp(20px,3vw,40px)', maxWidth: '1400px', margin: '0 auto' }}>
        <span className="sec-tag">Kategoriler</span>
        <h2 className="sec-h">Doğallığı <em>Keşfedin</em></h2>

        {/* CSS class ile grid — JS'e bağımlı değil */}
        <div className="cat-grid">
          {[
            ...
            { bg: 'linear-gradient(135deg,#FAF5FF,#E9D5FF)', emoji: '🎁', lbl: 'Hediye', ad: 'Hediye Seti', href: '/kampanyalar', first: false },
          ].map((k) => (
            <Link key={k.href} href={k.href} className={`card-2026${k.first ? ' cat-first' : ''}`} style={{ textDecoration: 'none', display: 'block' }}>
              <div style={{ background: k.bg, minHeight: k.first ? '460px' : '200px', height: '100%', display: 'flex', alignItems: 'flex-end', padding: '20px', position: 'relative' }}>
                <div style={{ position: 'absolute', top: '16px', right: '16px', fontSize: k.first ? '52px' : '36px' }}>{k.emoji}</div>
                <div>
                  ...
                  <div style={{ fontFamily: '"Instrument Serif", serif', fontSize: k.first ? '24px' : '18px', color: '#1A0A12' }}>{k.ad}</div>
      ...
      {/* ÜRÜNLER */}
      <section style={{ padding: 'clamp(24px,4vw,48px) clamp(16px,5vw,80px)', maxWidth: '1400px', margin: '0 auto' }}>
      ...
      {/* ABONELİK */}
      <section style={{ margin: '0 clamp(12px,4vw,80px) 60px', borderRadius: '40px', overflow: 'hidden', background: 'linear-gradient(135deg,#FEE8EF,#EBF5FC)', position: 'relative' }}>
        <div className="blob blob-p" style={{ width: '300px', height: '300px', top: '-80px', right: '-40px', opacity: .4 }} />
        <div className="banner-grid" style={{ padding: 'clamp(32px,5vw,72px)', position: 'relative', zIndex: 2 }}>
          <div>
            <span className="sec-tag">⟳ Haftalık Abonelik</span>
            <h2 style={{ fontFamily: '"Instrument Serif", serif', fontSize: 'clamp(26px,4vw,46px)', color: '#1A0A12', lineHeight: 1.1, marginBottom: '16px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {[{ a: 'Başlangıç', d: '2L · Haftada Bir', f: '520', hot: false }, { a: 'Aile', d: '4L · Haftada Bir', f: '980', hot: true }, { a: 'Premium', d: '6L · Haftada Bir', f: '1.380', hot: false }].map(p => (
              <div key={p.a} style={{ background: 'rgba(255,255,255,.88)', borderRadius: '20px', padding: '16px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', border: p.hot ? '2px solid #E8567A' : '2px solid transparent' }}>
                <div>
                  <div style={{ fontFamily: '"Instrument Serif", serif', fontSize: '18px', color: '#1A0A12' }}>
                    {p.a}{p.hot && <span style={{ fontSize: '9px', fontWeight: 800, background: '#E8567A', color: '#fff', padding: '2px 10px', borderRadius: '50px', marginLeft: '10px', letterSpacing: '.08em' }}>POPÜLER</span>}
                  </div>
      ...
      {/* YORUMLAR */}
      <section style={{ padding: 'clamp(40px,6vw,72px) clamp(16px,5vw,80px)', background: '#1A0A12' }}>
        <div style={{ maxWidth: '1240px', margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: '36px' }}>
            ...
            <h2 style={{ fontFamily: '"Instrument Serif", serif', fontSize: 'clamp(26px,4vw,46px)', color: '#fff' }}>Sizden <em style={{ fontStyle: 'italic', color: '#E8567A' }}>Gelenler</em></h2>
          </div>
          <div className="rev-grid">
            {[
              ...
              <div key={y.a} style={{ background: 'rgba(255,255,255,.06)', border: '1px solid rgba(255,255,255,.1)', borderRadius: '28px', padding: '24px', backdropFilter: 'blur(12px)' }}>
                <div style={{ display: 'flex', gap: '2px', marginBottom: '12px' }}>
                  {[1,2,3,4,5].map(s => <Star key={s} size={14} fill="#FBBF24" style={{ color: '#FBBF24' }} />)}
                </div>
                <p style={{ fontFamily: '"Instrument Serif", serif', fontStyle: 'italic', fontSize: '16px', lineHeight: 1.65, color: 'rgba(255,255,255,.85)', marginBottom: '18px' }}>{y.t}</p>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  <div style={{ width: '36px', height: '36px', borderRadius: '50%', background: 'linear-gradient(135deg,#E8567A,#5BA4CF)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, color: '#fff', fontSize: '13px' }}>{y.h}</div>
                  <div>
      ...
      {/* BÜLTEN */}
      <section style={{ padding: 'clamp(48px,7vw,88px) 24px', textAlign: 'center' }}>
        <h2 style={{ fontFamily: '"Instrument Serif", serif', fontSize: 'clamp(26px,4vw,48px)', color: '#1A0A12', marginBottom: '10px' }}>
        ...
        <form onSubmit={e => e.preventDefault()} style={{ display: 'flex', maxWidth: '420px', margin: '0 auto', background: 'rgba(26,10,18,.04)', border: '1.5px solid rgba(26,10,18,.1)', borderRadius: '16px', padding: '5px' }}>
          <input type="email" placeholder="E-posta adresiniz" style={{ flex: 1, background: 'transparent', border: 'none', padding: '12px 14px', fontSize: '14px', color: '#1A0A12', outline: 'none', fontFamily: 'var(--font-syne), Syne, sans-serif', minWidth: 0 }} />
